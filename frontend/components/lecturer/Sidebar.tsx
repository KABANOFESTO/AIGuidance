...
export default function Sidebar() {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            setUser({
                ...
                code: 'LEC001',
                avatarColor: '#22c55e', // green avatar as shown in screenshot
            });
    ...
    const initials = user?.fullName
        ?.split(' ')
        .filter((part) => /^[A-Za-z]/.test(part)) // skip "Dr." prefix dot issue
        ...
        <div className="flex h-full flex-col" style={{ background: '#1a237e' }}>
            ...
            <div className="flex items-center justify-between px-4 py-4">
                <div className="flex items-center gap-3 overflow-hidden">
                    ...
                    <div className="overflow-hidden">
                        ...
                        <p className="truncate text-xs font-medium" style={{ color: '#90caf9' }}>
                            Lecturer Portal
                        </p>
                    </div>
                </div>
                {/* Desktop hamburger icon (decorative, matching screenshot) */}
                <button
                    ...
                    <div className="overflow-hidden">
                        {loadingUser ? (
                            ...
                            <>
                                ...
                                <p className="truncate text-xs" style={{ color: '#90caf9' }}>{user?.code}</p>
                            </>
                        )}
                    </div>
                </div>
            ...
            <div className="border-t border-white/10 px-3 py-4">
                <Link
                    ...
                    onClick={() => setMobileOpen(false)}
                    className="flex items-center gap-3 rounded-full px-3.5 py-2.5 text-sm font-semibold text-slate-300 transition-colors hover:bg-white/10 hover:text-white"
                >
                ...
                <button
                    ...
                    className="flex w-full items-center gap-3 rounded-full px-3.5 py-2.5 text-sm font-semibold text-slate-300 transition-colors hover:bg-white/10 hover:text-white"
                >
